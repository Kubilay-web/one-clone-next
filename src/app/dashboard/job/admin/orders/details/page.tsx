  ...
  const fetchdata = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/admin/orders/details/${id}`,
      );
    ...
    <>
      ...
      >
        ...
        {/* {details.length > 0 ? (
          <PDFDownloadLink
            document={
              <Invoice order={details} company={companydata} plan={plandata} />
            }
            fileName={`invoice_${details[0]?.order_id || "order"}.pdf`}
